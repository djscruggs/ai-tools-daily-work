...
**Automated guardrails via hooks.** My Claude config runs shell hooks at key moments: Prettier auto-formats every file Claude touches, pre-commit type check and size check. direnv reloads environment variables when the working directory changes, and an audio ping signals when Claude finishes a long task.
